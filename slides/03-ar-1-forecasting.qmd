...
- In the small world of the AR(1), mainly the first two.
:::
...
So the conditional distribution has the same structure as the unconditional joint that we have already studied.
...
## Probabilistic forecasting via Monte Carlo {.medium}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("true_b0", "True b0 (intercept):", min = -1, max = 1, value = 0.5, step = 0.1),
      sliderInput("true_b1", "True b1 (AR coefficient):", min = -0.95, max = 0.95, value = 0.7, step = 0.05),
      sliderInput("true_sigma", "True sigma (innovation sd):", min = 0.1, max = 2, value = 1, step = 0.1),
...
$$
\begin{aligned}
\Btheta^{(j)} &\sim p(\boldsymbol{\theta}\,|\,y_{1:T})\\
y^{(j)}_{T+1:T+H}
&\sim 
\tilde{y}_{T+1:T+H}\,|\,y_{1:T},\,\Btheta^{(j)}), &&j = 1\com 2\com ...\com k.
\end{aligned}
$$

The retained sample $\tilde{y}^{(1:k)}_{T+1:T+H}$ is drawn from the posterior predictive.

## Same slide as before {.medium}
